    ...
    private var achivementsLayout: some View {
        VStack {
            Text("Your achievements")
                ...
                .padding(.top, 10)
            TabView {
                ForEach(0..<vm.achievements.count / 6 + (vm.achievements.count % 6 == 0 ? 0 : 1), id: \.self) { tabIndex in
                    VStack {
                        HStack {
                            ForEach(0..<3) { index in
                                let achievementIndex = tabIndex * 6 + index
                                if achievementIndex < vm.achievements.count {
                                    AchievementCard(achievement: vm.achievements[achievementIndex])
                                }
                            }
                        }
